...
export default function Experience() {
        ...
        setExperience([
          {
            ...
            technologies: ["React", "Next.js", "Node", "MongoDB"],
          },
        ]);
      } finally {
        setLoading(false);
  ...
  return (
    <section ref={containerRef} className="py-20 relative overflow-hidden">
      ...
      {/* Floating blobs - only these use the scroll transform */}
      <div className="absolute inset-0 opacity-20 pointer-events-none">
        <motion.div
          style={{ y, willChange: "transform" }}
          className="absolute top-20 left-10 w-72 h-72 bg-gradient-to-r from-purple-400/30 to-blue-600/30 rounded-xl blur-3xl"
        />
        <motion.div
          style={{ y, willChange: "transform" }}
          className="absolute bottom-20 right-10 w-96 h-96 bg-gradient-to-r from-blue-400/30 to-purple-600/30 rounded-xl blur-3xl"
        />
      </div>
        ...
        <div className="max-w-4xl mx-auto">
          {experience.map((exp, index) => {
            ...
            return (
              ...
              >
                <motion.div
                  className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-sm rounded-3xl p-8 shadow-lg hover:shadow-2xl border border-white/20 dark:border-slate-700/20 transition-all duration-400"
                ...
                >
                  <div className="flex flex-col gap-6 mb-6">
                    <div className="flex items-center gap-3 text-purple-600 dark:text-purple-400">
                      ...
                      <h3 className="font-bold text-xl text-slate-900 dark:text-white">{exp.title}</h3>
                    </div>

                    <p className="text-2xl font-semibold text-slate-900 dark:text-white">{exp.company}</p>
                  ...
                  <p className="text-slate-600 dark:text-slate-300 leading-relaxed mb-4 text-justify">{exp.description}</p>
                    ...
                    <>
                      ...
                      <div className="flex flex-wrap gap-2 mb-2">
                        {achievements.map((achievement, achIndex) => (
                          <motion.span
                            key={`ach-${achIndex}-${String(achievement).slice(0, 20)}`}
                            initial={{ opacity: 0, scale: 0.9 }}
                            whileInView={{ opacity: 1, scale: 1 }}
                            viewport={{ once: true }}
                            transition={{ duration: 0.35, delay: achIndex * 0.06 }}
                    ...
                    <div className="mt-4">
                      <div className="flex flex-wrap gap-2">
                        {technologies.map((tech, techIndex) => (
                          <motion.span
                            key={`tech-${techIndex}-${String(tech).slice(0, 12)}`}
                            initial={{ opacity: 0, scale: 0.9 }}
                            whileInView={{ opacity: 1, scale: 1 }}
                            viewport={{ once: true }}
